  ...
  for ln in lines:
    ...
    if trace_line.match(line):
      ...
      if area == UNKNOWN or area == HEAP or area == STACK:
        ...
        (source_symbol,
         source_location,
         object_symbol_with_offset) = symbol.SymbolInformation(os.path.basename(area), code_addr)
    ...
    if value_line.match(line):
      ...
      if area == UNKNOWN or area == HEAP or area == STACK or not area:
        ...
        (source_symbol,
         source_location,
         object_symbol_with_offset) = symbol.SymbolInformation(os.path.basename(area), value)
        if not source_location:
          source_location = ""
        if not object_symbol_with_offset:
